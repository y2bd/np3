...
class Rating(db.Model):
    ...
    album_art_url: so.Mapped[Optional[str]] = so.mapped_column(sa.VARCHAR(), nullable=True)
...
def get_latest_ratings(order_by, limit=1000):
    if order_by == 'artist':
        order_by_expr = Rating.artist.asc()
    ...
    for page in range(1, page_limit):
        ...
        for rating in ratings:
            existing_rating = db.session.execute(
                db.select(Rating).filter_by(artist=rating['artist'], album=rating['album'])
            ).scalar_one_or_none()
            if existing_rating:
